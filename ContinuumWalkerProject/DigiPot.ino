DigiPot::DigiPot(uint8_t incPin, uint8_t udPin, uint8_t csPin) {
  ...
  aw.pinMode(_udPin, OUTPUT);
  aw.pinMode(_csPin, OUTPUT);
  aw.digitalWrite(_csPin, HIGH);
...
void DigiPot::reset() {
  ...
  _currentValue = 0;
  Serial.println("Had to reset digipot");
}
...
void DigiPot::change(uint8_t direction, uint8_t amount) {
  amount = constrain(amount, 0, DIGIPOT_MAX_AMOUNT);
  aw.digitalWrite(_udPin, direction);
  digitalWrite(_incPin, HIGH);
  aw.digitalWrite(_csPin, LOW);

  for (uint8_t i=0; i<amount; i++) {
    digitalWrite(_incPin, LOW); //These pins need to NOT be on a GPIO expander.  
    delayMicroseconds(2);
    digitalWrite(_incPin, HIGH);
    delayMicroseconds(2);
